export default () => {
  ...
  const optionClass = "px-4 py-2.5 text-sm text-text-primary transition hover:bg-link-primary"
  ...
  const dropdownStyle = computed(() => {
    return {
      ...
      optionGroupLabel: {
        class: "px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-text-icon"
      }
    }
  })
  ...
  const multiSelectStyle = computed(() => {
    return {
      ...dropdownStyle.value,
      header: {
        class: "flex items-center justify-between gap-2 p-2 bg-inherit dropdown-text"
      },
      item: ({ context }: any) => ({
        class: context.selected
  ...
  const autocompleteStyle = computed(() => {
    return {
      ...
      inputToken: {
        class: "flex-1 placeholder:font-light placeholder:text-placeholder-primary"
      },
      panel: {
        class: `${overlayClass} max-h-[220px] overflow-y-auto w-[250px] xl:w-[350px]`
      },
      header: {
        class: "border-b border-border-primary bg-bg-secondary py-3.5 text-text-primary"
      },
      container: {
  ...
  const dialogStyle = computed(() => {
    return {
      ...
      header: {
        class: "p-5 flex justify-between items-center border-b border-border-primary"
      },
      headerTitle: {
        class: "font-semibold"
      },
      ...
      footer: {
        class: "p-3 border-t border-border-primary flex justify-end items-center"
      }
    }
  })
